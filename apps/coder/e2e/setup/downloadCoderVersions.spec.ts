...
import { downloadOptimus IDE CollabVersion } from "../helpers";
...
test("download outdated Optimus IDE Collab CLI", async () => {
	...
	// downloadOptimus IDE CollabVersion itself, so it retries inline and fails alone.
	try {
		await downloadOptimus IDE CollabVersion(oldestSupportedCLIVersion);
	} catch (error) {
		console.error(
			`Failed to prefetch the Optimus IDE Collab ${oldestSupportedCLIVersion} CLI. ` +
