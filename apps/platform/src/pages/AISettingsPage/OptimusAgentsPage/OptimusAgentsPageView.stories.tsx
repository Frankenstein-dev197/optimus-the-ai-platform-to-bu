...
import {
	...
	type OptimusAgentsPageViewProps,
} from "./OptimusAgentsPageView";
...
const buildArgs = (
	overrides: Partial<OptimusAgentsPageViewProps> = {},
): OptimusAgentsPageViewProps => ({
...
const meta = {
	title: "pages/AISettingsPage/OptimusAgentsPage/OptimusAgentsPageView",
	component: OptimusAgentsPageView,
	// TODO: Stories in this file fail when pixel runs their play functions. Fix them and remove the exclude.
	parameters: { pixel: { exclude: true } },
	args: buildArgs(),
} satisfies Meta<typeof OptimusAgentsPageView>;

export default meta;
type Story = StoryObj<typeof OptimusAgentsPageView>;
...
export const AllOverridesUnset: Story = {
	...
	play: async ({ canvasElement }) => {
		const canvas = within(canvasElement);
		expect(
			await canvas.findByRole("heading", { name: "Optimus Agents" }),
		).toBeVisible();
		expect(
			canvas.getByText(
		...
		await canvas.findByText(
			"Leave unset to use Optimus's title default, which prefers fast models from configured providers.",
		);
