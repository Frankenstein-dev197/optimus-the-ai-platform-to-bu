...
interface OptimusIDECollabnaut {
	x: number;
	padIdx: number;
	dir: 1 | -1;
	speed: number;
...
// How many OptimusIDECollabnauts appear per round.
const OPTIMUS_IDE_COLLABNAUTS_PER_ROUND = 10;
...
// ---------------------------------------------------------------------------
// OptimusIDECollabnauts - little astronauts stranded on the landing pads
// ---------------------------------------------------------------------------
...
function createOptimusIDECollabnauts(
	pads: LandingPad[],
	entries: { name: string; role: string }[],
): OptimusIDECollabnaut[] {
	const nauts: OptimusIDECollabnaut[] = [];
...
function updateOptimusIDECollabnauts(nauts: OptimusIDECollabnaut[], pads: LandingPad[], dt: number) {
...
/**
 * Draw a single OptimusIDECollabnaut at their position on the pad surface.
 * The figure is ~10 px tall: round helmet, stick body, animated
 * legs and arms. Drawn in gray to match the station aesthetic.
 */
function drawOptimusIDECollabnaut(
	ctx: CanvasRenderingContext2D,
	naut: OptimusIDECollabnaut,
	groundY: number,
) {
	const color = "#888";
...
function drawTerrain(ctx: CanvasRenderingContext2D, terrain: Terrain) {
	...
	for (const pad of pads) {
		...
		if (pad.isStation) {
			...
			drawVectorText(ctx, "OPTIMUS_IDE_COLLAB BASE", midX, pad.y + tick + 10, 7);
		} else {
...
// ---------------------------------------------------------------------------
// Disembark animation - OptimusIDECollabnauts walking into the base
// ---------------------------------------------------------------------------
...
function drawSidebar(
	...
	ctx.textAlign = "left";
	ctx.fillText("OPTIMUS_IDE_COLLABNAUTS ROSTER", x + padX, headerH - 6);
...
export const LunarLander: FC = () => {
	...
	useEffect(() => {
		...
		let optimusIdeCollabnauts: OptimusIDECollabnaut[] = [];
		...
		function startNewRound() {
			...
			const pick = unsaved.slice(0, OPTIMUS_IDE_COLLABNAUTS_PER_ROUND);
			terrain = generateTerrain(logicalW - SIDEBAR_W, logicalH - DASHBOARD_H);
			optimusIdeCollabnauts = createOptimusIDECollabnauts(terrain.pads, pick);
			disembarks = [];
			resetLander();
		...
		function resetLander() {
			...
			explosionParts = [];
			fuel = 100;
			for (const n of optimusIdeCollabnauts) {
		...
		function onClick(e: MouseEvent) {
			...
			if (dialogVisible) {
				...
				if (
					...
					// Yes - travel to another base with this OptimusIDECollabnaut.
					...
					if (entry) {
						...
						const pick = [entry, ...unsaved.slice(0, OPTIMUS_IDE_COLLABNAUTS_PER_ROUND - 1)];
			...
			const onScreen = optimusIdeCollabnauts.find(
		...
		function loop(ts: number) {
			...
			if (introTimer > 0) {
				...
				drawVectorText(cx, "OPTIMUS_IDE_COLLABNAUTS", dashW / 2, playH * 0.43, 44);
			...
			if (victory) {
				...
				cx.textAlign = "center";
				cx.fillText("All OptimusIDECollabnauts have been saved!", dashW / 2, playH * 0.6);
			...
			// -- update optimusIdeCollabnauts --
			if (terrain) updateOptimusIDECollabnauts(optimusIdeCollabnauts, terrain.pads, dt);
					...
					if (
						...
						if (pad) {
							...
							if (pad.isStation) {
								...
								const aboardCount = optimusIdeCollabnauts.filter((c) => c.aboard).length;
							...
							if (optimusIdeCollabnauts.every((c) => c.saved)) {
			...
			if (terrain) {
				...
				for (const naut of optimusIdeCollabnauts) {
					if (!naut.aboard && !naut.saved) {
						drawOptimusIDECollabnaut(cx, naut, terrain.pads[naut.padIdx].y);
					}
				}
			}
			...
			for (const a of disembarks) {
				const tmpNaut: OptimusIDECollabnaut = {
					x: a.x,
					padIdx: 0,
				...
				drawOptimusIDECollabnaut(cx, tmpNaut, a.groundY);
			}
			...
			const currentNautNames = new Set(
				optimusIdeCollabnauts.filter((c) => !c.aboard && !c.saved).map((c) => c.name),
			);
			drawSidebar(
				cx,
				logicalW,
			...
			const cargo = optimusIdeCollabnauts.filter((c) => c.aboard);
			const savedPct = (savedNames.size / ROSTER.length) * 100;
